        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            try
            {
                using (var _context = new DatabaseContext())
                {
                    ...
                    //}
                    //Send EmployeeId as context.UserName
                    ...
                    int empId = Convert.ToInt32(context.UserName);
                    var teamIDUserName = await _context.Employee.Where(x => x.BSIPLid == empId)
                        .Select(x => new { x.TeamId, x.Name }).FirstOrDefaultAsync();

                    if (user != null)
                    {
                        var identity = new ClaimsIdentity(context.Options.AuthenticationType);
                        identity.AddClaim(new Claim("Username", teamIDUserName.Name)); //get username from Employee Table
                        ...
                        {
                            "empid",Newtonsoft.Json.JsonConvert.SerializeObject(empId)
                        },
                        {
                            "teamid",Newtonsoft.Json.JsonConvert.SerializeObject(teamIDUserName.TeamId)
                        }
                        });
